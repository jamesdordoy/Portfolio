...
describe('Project', () => {
    it('renders the project name', () => {
        const wrapper = mount(Project, {
            global: globalConfig(),
            props: { project: makeProject({ name: 'My App' }) },
        });
    ...
    it('renders the project description', () => {
        const wrapper = mount(Project, {
            global: globalConfig(),
            props: { project: makeProject({ description: 'An awesome app' }) },
        });
    ...
    it('links to the project url', () => {
        const wrapper = mount(Project, {
            global: globalConfig(),
            props: { project: makeProject({ link: 'https://example.com' }) },
        });
        const links = wrapper.findAll('a');
        expect(links.some((a) => a.attributes('href') === 'https://example.com')).toBe(true);
    });

    it('renders the project icon', () => {
        const wrapper = mount(Project, {
            global: globalConfig(),
            props: { project: makeProject({ icon: '/images/myapp.png' }) },
        });
        expect(wrapper.find('img').attributes('src')).toBe('/images/myapp.png');
    });

    it('falls back to the default icon when icon is empty', () => {
        const wrapper = mount(Project, {
            global: globalConfig(),
            props: { project: makeProject({ icon: '' }) },
        });
